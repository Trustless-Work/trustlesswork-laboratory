...
In all of your chat prompts, you should provide the agent with specific document references, including the global content rules and the complete code project. **_GPT 5 gives the best result_**
...
- Create a TrustlessWorkProvider client component wrapping TrustlessWorkConfig from @trustless-work/escrow.
  Use the `development` base URL constant for testnet (or `mainNet` for production) and TW_API_KEY env var.
